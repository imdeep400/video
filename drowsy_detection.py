...
def draw_transparent_circle(frame, center, radius, color, alpha):
...
def plot_eye_landmarks(frame, left_lm_coordinates, right_lm_coordinates, color,fs=1,transp=False):
        ...
        if lm_coordinates:
            for coord in lm_coordinates:
                if transp:
                    alpha=0.4  
                    overlay = frame.copy()
                    cv2.circle(overlay, coord, fs, color, -1)
                    cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)
                else:
                    cv2.circle(frame, coord, fs, color, -1)
...
def plot_text(image, text, origin, color, font=cv2.FONT_HERSHEY_SIMPLEX, fs=0.8, thickness=2, frm_i=0):
    thickness2 = int(2*thickness*(math.sin(math.radians(frm_i*20))+2))
    # print(f'{thickness=} {thickness2=} {frm_i=} {math.sin(math.radians(frm_i))=}')
    image = cv2.putText(image, text, origin, font, fs, 0, thickness2)    
    image = cv2.putText(image, text, origin, font, fs, color, thickness)
    return image
